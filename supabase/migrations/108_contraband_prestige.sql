-- =====================================================
-- CONTRABAND PRESTIGE ENHANCEMENT
-- =====================================================
-- 1. Respect bonus for consumable-based PvP attacks
-- 2. Family contraband power calculation

SET search_path = public;

-- =====================================================
-- 1. UPDATE PERFORM_PVP_ATTACK: RESPECT BONUS
-- =====================================================
-- Consumable attacks grant +10 respect, non-consumable +5
...
BEGIN
    ...
    IF attack_type.requires_consumables THEN
        respect_bonus := 10;  -- Higher prestige for consumable attacks
    ELSE
        respect_bonus := 5;   -- Standard bonus
    END IF;
    ...
    IF attacker_wins THEN
        ...
        -- Apply gains/losses (using dynamic respect_bonus)
...
-- =====================================================
-- 2. FAMILY CONTRABAND POWER
-- =====================================================

-- Add contraband_power column to families
ALTER TABLE public.families 
ADD COLUMN IF NOT EXISTS contraband_power BIGINT DEFAULT 0;

-- Function to calculate family contraband power from inventory
CREATE OR REPLACE FUNCTION calculate_family_contraband_power(family_id_input UUID)
RETURNS BIGINT AS $$
DECLARE
    ...
    item_value INTEGER;
BEGIN
    SELECT inventory INTO inv FROM public.families WHERE id = family_id_input;
    
    IF inv IS NULL OR inv = '{}'::jsonb THEN 
        RETURN 0; 
    END IF;
    
    FOR item_key, item_qty IN SELECT * FROM jsonb_each_text(inv)
    LOOP
        -- Only count contraband items
        SELECT sell_price INTO item_value 
        FROM public.item_definitions 
        WHERE id = item_key::UUID AND category = 'contraband';
        
        IF item_value IS NOT NULL THEN
            total_power := total_power + (item_value * item_qty::INTEGER);
        END IF;
    END LOOP;
...
ALTER FUNCTION public.calculate_family_contraband_power(UUID) SET search_path = public;

COMMENT ON FUNCTION calculate_family_contraband_power IS 'Calculates total value of contraband stored in family inventory for war power';


-- =====================================================
-- 3. GET FAMILY POWER RPC (includes contraband)
-- =====================================================
CREATE OR REPLACE FUNCTION get_family_power(family_id_input UUID)
RETURNS JSONB AS $$
DECLARE
    fam RECORD;
    member_count INTEGER;
    total_respect BIGINT;
    contraband_power BIGINT;
BEGIN
    SELECT * INTO fam FROM public.families WHERE id = family_id_input;
    IF fam IS NULL THEN
        RETURN jsonb_build_object('success', false, 'message', 'Family not found');
    END IF;
    
    -- Count members and total respect
    SELECT COUNT(*), COALESCE(SUM(p.respect), 0)
    INTO member_count, total_respect
    FROM public.family_members fm
    JOIN public.players p ON p.id = fm.player_id
    WHERE fm.family_id = family_id_input;
    
    -- Calculate contraband power
    contraband_power := calculate_family_contraband_power(family_id_input);
    ...
    RETURN jsonb_build_object(
        ...
        'contraband_power', contraband_power,
        'treasury', fam.treasury,
        'total_power', total_respect + contraband_power
    );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

ALTER FUNCTION public.get_family_power(UUID) SET search_path = public;

COMMENT ON FUNCTION get_family_power IS 'Returns family power stats including contraband value for wars';
