-- =====================================================
-- PVP RESPECT LOGIC ALIGNMENT
-- =====================================================
-- Aligns PvP Respect mechanics with design rules:
-- 1. Shield blocks PvP entirely (no combat, no Respect change)
-- 2. Attacker loss: flat -4 to -8 (not ratio-based)
-- 3. Defender success: +2 (not +3)
-- 4. Insurance does NOT protect Respect (already correct)
-- 5. RPC returns explicit respect_gained AND respect_lost

SET search_path = public;

-- Drop and recreate with aligned logic
...
BEGIN
    ...
    -- =====================================================
    -- Per design: Shield blocks PvP - no losses, no Respect change
    IF has_shield THEN
        RETURN jsonb_build_object(
            'success', false,
    ...
    -- Shield no longer doubles defense - it blocks PvP entirely (handled above)
    ...
    ELSE
        ...
        -- Design rule: NOT scaled with damage, crew size, or power
        attacker_respect_loss := 4 + floor(random() * 5)::INTEGER;  -- Random 4-8
