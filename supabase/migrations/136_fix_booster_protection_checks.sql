-- =====================================================
-- FIX BOOSTER AND PROTECTION ISSUES IN PVP
-- =====================================================
-- 1. Add protection_expires_at check (purchased protection packages)
-- 2. Add VIP pass shield check (VIP includes shield protection)
-- 3. Add VIP pass attack boost check (VIP includes 2x attack)
-- =====================================================

SET search_path = public;

-- Drop and recreate perform_pvp_attack with all fixes
...
DECLARE
    ...
    has_vip BOOLEAN := false;  -- NEW: Track VIP separately
...
BEGIN
    ...
    -- =====================================================
    -- PROTECTION CHECK 2: PURCHASED PROTECTION (FIX!)
    -- =====================================================
    ...
    SELECT EXISTS (
        ...
        AND booster_type IN ('2x_attack', 'vip_pass')  -- FIX: Include VIP!
        AND expires_at > NOW()
    ...
    SELECT EXISTS (
        ...
        AND booster_type IN ('shield', 'vip_pass')  -- FIX: Include VIP!
        AND expires_at > NOW()
    ) INTO has_shield;
    ...
    ELSE
        ...
        -- Attacker loses some crew on failed attack
...
COMMENT ON FUNCTION perform_pvp_attack(UUID, UUID, TEXT, BOOLEAN, UUID) IS 
    'PvP attack with full protection checks: newbie shield, purchased protection, shield booster, VIP pass.';
