...
BEGIN
    ...
    -- Check if already completed (unless specific logic for repeatable exists, assuming one-time for now)
    -- We check the player_tasks table
    ...
    -- Mark as completed (Upsert to handle "started" but not completed state if any)
