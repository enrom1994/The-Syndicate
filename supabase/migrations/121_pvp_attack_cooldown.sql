-- =====================================================
-- PVP ATTACK COOLDOWN SYSTEM
-- =====================================================
-- Prevents harassment by introducing 3-hour per-attacker-per-target cooldown
-- 
-- RULES:
--   - Player cannot attack same target again for 3 hours
--   - Cooldown is per attacker → per target (can attack others)
--   - Applies to Drive-By PvP attacks AND Revenge attacks
--   - Taking revenge starts a 3h cooldown vs that target
--   - Protection/Shield check happens BEFORE cooldown check
--
-- VALIDATION ORDER:
--   1. Target has Protection → attack blocked
--   2. Cooldown active → attack blocked (PVP_COOLDOWN_ACTIVE)
--   3. Cash fee available → proceed

SET search_path = public;

-- =====================================================
-- 1. ADD INDEX FOR COOLDOWN LOOKUP
-- =====================================================
-- Optimizes the cooldown check query

CREATE INDEX IF NOT EXISTS idx_attack_log_cooldown 
ON public.attack_log(attacker_id, defender_id, created_at DESC);

COMMENT ON INDEX idx_attack_log_cooldown IS 'Optimizes per-attacker per-target cooldown lookups';

-- =====================================================
-- 2. HELPER FUNCTION: CHECK PVP COOLDOWN
-- =====================================================
-- Returns cooldown status between two players
...
DECLARE
    ...
    cooldown_duration INTERVAL := INTERVAL '3 hours';
    cooldown_ends_at TIMESTAMPTZ;
    remaining_seconds INTEGER;
BEGIN
    -- Find the most recent attack from this attacker to this target
    SELECT created_at INTO last_attack_at
    ...
    -- No previous attack = no cooldown
    IF last_attack_at IS NULL THEN
    ...
    cooldown_ends_at := last_attack_at + cooldown_duration;
    
    -- Check if still on cooldown
    IF NOW() < cooldown_ends_at THEN
        remaining_seconds := EXTRACT(EPOCH FROM (cooldown_ends_at - NOW()))::INTEGER;
        RETURN jsonb_build_object(
            'on_cooldown', true,
...
ALTER FUNCTION public.check_pvp_cooldown(UUID, UUID) SET search_path = public;

COMMENT ON FUNCTION check_pvp_cooldown IS 'Checks if attacker is on cooldown for attacking a specific target. 3-hour cooldown per attacker-target pair.';

-- =====================================================
-- 3. UPDATE PERFORM_PVP_ATTACK WITH COOLDOWN CHECK
-- =====================================================
...
BEGIN
    ...
    SELECT 
        ...
        -- Check if we're on cooldown against this attacker
        COALESCE(
            (
                SELECT (cs->>'on_cooldown')::BOOLEAN
                FROM check_pvp_cooldown(player_id_input, al.attacker_id) cs
            ),
            false
        ) AS on_cooldown,
        ...
        COALESCE(
            (
                SELECT (cs->>'remaining_seconds')::INTEGER
                FROM check_pvp_cooldown(player_id_input, al.attacker_id) cs
            ),
            0
...
COMMENT ON FUNCTION get_revenge_targets IS 'Returns eligible revenge targets with cooldown status. 3-hour cooldown applies after any attack.';

-- =====================================================
-- 5. UPDATE GET_PVP_TARGETS WITH COOLDOWN STATUS
-- =====================================================
-- Updates existing function to include cooldown information
...
BEGIN
    ...
    SELECT 
        ...
        COALESCE(
            ...
            (SELECT SUM(idef.defense_bonus * pi.quantity)::INTEGER 
             FROM player_inventory pi 
             JOIN item_definitions idef ON pi.item_id = idef.id 
        ...
        COALESCE(
            ...
            (SELECT SUM(idef.attack_bonus * pi.quantity)::INTEGER 
             FROM player_inventory pi 
             JOIN item_definitions idef ON pi.item_id = idef.id 
             WHERE pi.player_id = p.id), 0
        ) AS attack,
        ...
        -- NPP check
        (p.newbie_shield_expires_at > NOW()) AS has_npp,
        -- Cooldown check
        COALESCE(
            (check_pvp_cooldown(player_id_input, p.id)->>'on_cooldown')::BOOLEAN,
            false
        ) AS on_cooldown,
        COALESCE(
            (check_pvp_cooldown(player_id_input, p.id)->>'remaining_seconds')::INTEGER,
            0
        ) AS cooldown_remaining
    FROM public.players p
...
COMMENT ON FUNCTION get_pvp_targets IS 'Returns PvP targets with Made Man, shield, NPP, and 3-hour cooldown status';
