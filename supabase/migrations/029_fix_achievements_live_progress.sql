-- =====================================================
-- FIX ACHIEVEMENTS - LIVE PROGRESS CALCULATION
-- =====================================================
-- Replaces the static progress from player_achievements with
-- live calculations from actual game state.
...
BEGIN
    ...
    FROM attack_log 
    WHERE attacker_id = target_player_id AND result = 'victory';
    
    -- Count successful defenses (attacker lost)
    SELECT COUNT(*) INTO defense_wins 
    FROM attack_log 
    WHERE defender_id = target_player_id AND result = 'defeat';
...
-- Also update claim_achievement to sync the player_achievements table when claiming
...
BEGIN
    ...
    SELECT COUNT(*) INTO attack_wins FROM attack_log WHERE attacker_id = claimer_id AND result = 'victory';
    SELECT COUNT(*) INTO defense_wins FROM attack_log WHERE defender_id = claimer_id AND result = 'defeat';
